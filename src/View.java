...
{ 
    ...
    public void drawActualPicture( Graphics2D g )
    {
        ...
        synchronized( Model.class )   // Make thread safe
        {
            ...
            displayGameObj2( g, bat  );   // Display the Bat
            ...
            for (GameObj brick : bricks) 
            {
                if (brick.isVisible() == true) {
                    displayGameObj2(g, brick);
                }
            }
            for (GameObj brick : twoBricks) 
            {
                if (brick.isVisible() == true) {
                    displayGameObj2(g, brick);
                }
    ...
    private void displayGameObj2( Graphics2D g, GameObj go )
    {
        g.setColor( go.getColour().forSwing() );
